...
1. **所有修改或输出的新内容必须另存为新版本**：每次改动都创建新的 git 提交，不覆盖、不删除历史版本
2. 禁止使用 `git commit --amend`、`git rebase`、`git push --force` 等覆盖历史的操作
3. 提交信息需用中文简要说明本次改动内容（改了什么、新增了什么）
4. 改动完成后推送 GitHub，并验证公网地址正常（http 200）
